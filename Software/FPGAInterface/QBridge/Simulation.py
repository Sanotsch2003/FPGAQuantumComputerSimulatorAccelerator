...
class QiskitQCSimulator(QuantumCircuitSimulator):
    ...
    def run(self):
        ...
        transpiled_qc = transpile(self.qc, statevector_simulator)
...
class CustomQCSimulator(QuantumCircuitSimulator):
    ...
    def _operation(self, matrix_2x2, target, control=None, verbose=True):
        if verbose:
            print(f"Operation: {matrix_2x2}, target: {target}, control: {control}")
        for i in range(0, 2**(self.nQbits-1)):
            index_a, index_b = self._getElementsAandB(i, target)
            if verbose:
                print(f"iteration: {i} index_a: {index_a}, index_b: {index_b}")
