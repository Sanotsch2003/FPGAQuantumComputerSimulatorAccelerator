...
entity uartTransmitter is
 generic (
  ...
  Port ( 
    ...
    constant C_TICKS_PER_BIT : natural := clockFrequency / baudRate; 
    ...
    resized_data(resized_data'length - 1 downto resized_data'length - PRECISION) <= data;
        ...
        if rising_edge(clk) then
            ...
            if reset = '1' then
                ...
                if packet_counter < (C_PACKETS_PER_NUMBER + 2) and tx_start = '1' then
                    tx_busy <= '1';
                    if tick_counter < C_TICKS_PER_BIT then
                        ...
                        if bit_counter = 0 then
                            if packet_counter = 0 then --start of transmission
                                shift_reg(9 downto 0) <= "1000000010";
                                bit_counter <= bit_counter + 1;
                            
                            elsif packet_counter = C_PACKETS_PER_NUMBER + 1 then --end of transmission
                                shift_reg(9 downto 0) <= "1100000010";
                                bit_counter <= bit_counter + 1;
                            
                            else --during transmission
                                shift_reg(0) <= '0';
                                shift_reg(1) <= '0';
